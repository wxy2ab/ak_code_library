...
def get_version_from_github_file(proxies=None):
    url = "https://raw.githubusercontent.com/wxy2ab/ak_code_library/main/__init__.py"
...
def extract_and_replace(zip_filename, extract_to):
    ...
    temp_dir = os.path.join(extract_to, 'ak_code_library-main')
    ...
    # 输出更新后的 ./__init__.py 内容
    init_path = os.path.join(os.path.dirname(extract_to), '__init__.py')
    if os.path.exists(init_path):
        with open(init_path, 'r') as file:
            content = file.read()
            print(f"\nUpdated {init_path} content:\n")
            print(content)
    else:
...
def main():
    ...
    local_file_path = os.path.join(base_dir, '__init__.py')
    ...
    if parse_version(github_version) > parse_version(local_version):
        ...
        zip_url = "https://github.com/wxy2ab/ak_code_library/archive/refs/heads/main.zip"
        output_dir = os.path.join(base_dir, 'output')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        local_zip_filename = os.path.join(output_dir, "ak_code_library.zip")
